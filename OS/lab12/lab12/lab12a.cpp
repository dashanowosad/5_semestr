...
void Import(IMAGE_NT_HEADERS *pNtHdr, LPVOID pSrcFile){
	...
	while(ImportTable->Name != NULL){
		...
		sName=(char*)ImageRvaToVa(pNtHdr,pSrcFile,ImportTable->Name,NULL); //названи€ библиотек dll
...
int main(){
	...
	pSrcFile = (PBYTE) MapViewOfFile(hFileMap,FILE_MAP_READ,0,0,0); //получаем указатель на участок пам€ти (базовый)
	if(pSrcFile == NULL)
	...
	system("Start  Z:\\”нивер\\5_semestr\\OS\\lab12\\Malkov\\lab12\\lab12b.exe");
	Sleep(20);
